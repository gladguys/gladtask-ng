...
export class TaskService extends BaseService<Task> {
  ...
  findTasksByTargetUser(userId: String): Observable<Task[]> {
    return this.http.get<Task[]>(
      `${environment.API}/tasks/user-target/${userId}`
    );
  }

  findTasksByProject(projectId: String): Observable<Task[]> {
    return this.http.get<Task[]>(
      `${environment.API}/tasks/project/${projectId}`
    );
  }

  findTasksByTargetUserAndStatus(
    userId: String,
    status: string
  ): Observable<Task[]> {
    return this.http.get<Task[]>(
      `${environment.API}/tasks/user-target/${userId}/${status}`
    );
  }

  findFirst4ByTargetUserIdOrderByLastEdited(
    userId: String
  ): Observable<Task[]> {
    return this.http.get<Task[]>(
      `${environment.API}/tasks/last-edited/${userId}`
    );
  }

  findTasksByTargetUserAndProject(
    userId: string,
    projectId: string
  ): Observable<Task[]> {
    return this.http.get<Task[]>(
      `${environment.API}/tasks/project/${userId}/${projectId}`
    );
  }

  findTasksByCreatorUser(userId: String): Observable<Task[]> {
    return this.http.get<Task[]>(
      `${environment.API}/tasks/user-creator/${userId}`
    );
  }

  findBetweenDates(days: number, userId: string): Observable<Task[]> {
    return this.http.get<Task[]>(
      `${environment.API}/tasks/between/${days}/${userId}`
    );
  }
  ...
  updateTaskStatus(
    ...
    return this.http.put<Task>(
      `${environment.API}/tasks/${taskId}/update-status/${status}`,
      this.gladService.getIgnoreLoaderParam(ignoreLoader)
    );
  }

  findTasksLookAlikeByTitle(
    title: string,
    ignoreLoader: boolean = false
  ): Observable<Task[]> {
    return this.http.get<Task[]>(
      `${environment.API}/tasks/similar/title/${title}`,
      this.gladService.getIgnoreLoaderParam(ignoreLoader)
    );
  }
  ...
  saveTaskComment(
    ...
    return this.http.post<Task>(
      `${environment.API}/tasks/${id}/save-comment`,
      taskComment,
      this.gladService.getIgnoreLoaderParam(ignoreLoader)
    );
  }
  ...
  saveTimeSpent(
    ...
    return this.http.post<Task>(
      `${environment.API}/tasks/${id}/save-time-spent`,
      timeSpent,
      this.gladService.getIgnoreLoaderParam(ignoreLoader)
    );
  }
  ...
  saveTaskChange(
    ...
    ignoreLoader: boolean = false
  ): Observable<TaskChange> {
    return this.http.post<TaskChange>(
      `${environment.API}/tasks/${id}/save-task-change`,
      taskChange,
      this.gladService.getIgnoreLoaderParam(ignoreLoader)
    );
  }
