...
export class DateProximityPipe extends DatePipe implements PipeTransform {
  transform(value: any): any {
    ...
    let dueDateMilli = new Date(
      datePipe.transform(dueDate, 'yyyy-MM-dd')
    ).getTime() as number;

    let today = new Date();
    let todayDateMilli = new Date(
      datePipe.transform(today, 'yyyy-MM-dd')
    ).getTime() as number;
    ...
    if (diffDays >= 0) {
      switch (diffDays) {
        ...
        case 1:
          return 'Amanhã';
        default:
          return datePipe.transform(dueDate, 'dd-MM-yyyy');
      }
    }
    return '';
  }
