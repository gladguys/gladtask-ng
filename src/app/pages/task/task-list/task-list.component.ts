...
export class TaskListComponent implements OnInit {
  ...
  getTasksDueToday(tasks: Task[]): Task[] {
    return tasks.filter(
      (task) =>
        this.datePipe.transform(task.dueDate, 'yyyy-MM-dd') === this.today
    );
  }

  getTasksNotDueToday(tasks: Task[]): Task[] {
    return tasks.filter((task) => {
      let taskDueDateMilli = new Date(
  ...
  getTasksDelayed(tasks: Task[]): Task[] {
    return tasks.filter((task) => {
      ...
      return (
        taskDueDateMilli < this.todayDateMilli &&
  ...
  getTasksFinished(tasks: Task[]): Task[] {
    return tasks
      .filter((task) => task.status === Status.CONCLUIDA)
      .slice(0, 10);
  }
