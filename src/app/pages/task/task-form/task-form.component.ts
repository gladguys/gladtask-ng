...
import {
  possibleTaskTypes,
  TaskType,
} from '../../../shared/enums/task-type.enum';
...
export class TaskFormComponent implements OnInit {
  ...
  private getPossibleOptions() {
    ...
    this.possibleTeams$ = this.teamService.findAllByUser(
      this.sharedService.getUserLogged()._id
    );
  }
  ...
  checkAndPersistChanges(submittedTask: Task) {
    if (submittedTask.title !== this.task.title) {
      let taskChangeTitulo = this.buildTaskChange(
        'Título',
        this.task.title,
        submittedTask.title
      );
      this.taskService
        .saveTaskChange(submittedTask._id, taskChangeTitulo)
        .subscribe((t) => {});
    }
    if (submittedTask.description !== this.task.description) {
      let taskChangeDescription = this.buildTaskChange(
        'Descrição',
        this.task.description,
        submittedTask.description
      );
      this.taskService
        .saveTaskChange(submittedTask._id, taskChangeDescription)
        .subscribe((t) => {});
    }
    if (submittedTask.status !== this.task.status) {
      let taskChangeStatus = this.buildTaskChange(
        'Situação',
        this.task.status,
        submittedTask.status
      );
      this.taskService
        .saveTaskChange(submittedTask._id, taskChangeStatus)
        .subscribe((t) => {});
    }
    if (submittedTask.taskType !== this.task.taskType) {
      let taskChangeType = this.buildTaskChange(
        'Tipo',
        this.task.taskType,
        submittedTask.taskType
      );
      this.taskService
        .saveTaskChange(submittedTask._id, taskChangeType)
        .subscribe((t) => {});
    }
    if (submittedTask.priority !== this.task.priority) {
      let taskChangePriority = this.buildTaskChange(
        'Prioridade',
        this.task.priority,
        submittedTask.priority
      );
      this.taskService
        .saveTaskChange(submittedTask._id, taskChangePriority)
        .subscribe((t) => {});
    }
  ...
  onSubmit() {
    ...
    let submittedTask = this.taskForm.getRawValue() as Task;
    ...
    this.taskService.createOrUpdate(submittedTask).subscribe(
      (task) => {
        this.task = task;
        if (!isEdit) {
      ...
      (e) => {
        if (submittedTask._id != null) {
          this.notificationService.notificateFailure('Falha ao atualizar task');
        } else {
          this.notificationService.notificateFailure('Falha ao criar task');
        }
      }
    );
  }
  ...
  getTaskTypeDescription(taskType: TaskType): string {
    switch (taskType) {
      ...
      case TaskType.REUNIAO:
        return 'Reunião';
      case TaskType.MELHORIA:
        return 'Melhoria';
      case TaskType.FEATURE:
        return 'Feature';
      case TaskType.BUG:
        return 'Bug';
      case TaskType.ALINHAMENTO:
        return 'Alinhamento';
      case TaskType.DOCUMENTACAO:
        return 'Documentação';
      case TaskType.OUTRO:
        return 'Outro';
    }
  ...
  getLookAlikeTasksByTitle(title: string): any {
    this.taskService
      .findTasksLookAlikeByTitle(title, true)
      .subscribe((tasks) => {
  ...
  loadUsers(teamId: string) {
    this.userService
      .findByTeam(teamId)
      .subscribe((users) => (this.possibleTargetUsers = users));
  }

  loadProjects(teamId: string) {
    this.projectService.findAllByTeam(teamId, true).subscribe((projects) => {
      this.possibleProjects = projects;
    });
  ...
  addComment() {
    ...
    if (this.task._id !== undefined) {
      ...
      this.taskService
        .saveTaskComment(this.task._id, taskComment, true)
        .subscribe((t) => {});
    }
