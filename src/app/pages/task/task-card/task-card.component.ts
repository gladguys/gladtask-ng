  ...
  setupFormValueChanges(): void {
    if (this.canEdit) {
      this.taskForm.controls['title'].valueChanges
        ...
        .subscribe((value) => {
          this.task.taskChanges.push(
            this.buildTaskChange('Título', this.task.title, value)
          );
          this.task.title = value;
          this.taskService.createOrUpdate(this.task).subscribe(
            () => this.gladService.openSnack('task editada'),
            (e) =>
              this.notificationService.notificateFailure(
      ...
      this.taskForm.controls['description'].valueChanges
        ...
        .subscribe((value) => {
          this.task.taskChanges.push(
            this.buildTaskChange('Descrição', this.task.description, value)
          );
      ...
      this.taskForm.controls['status'].valueChanges.subscribe((value) => {
        this.task.taskChanges.push(
          this.buildTaskChange('Situação', this.task.status, value)
        );
        this.task.status = value;
        this.taskService.createOrUpdate(this.task).subscribe(
          () => this.gladService.openSnack('task editada'),
          (e) =>
            this.notificationService.notificateFailure('Falha ao criar equipe')
        );
      });
    }
  }
