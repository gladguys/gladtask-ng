...
export class TaskDetailComponent implements OnInit {
  ...
  remover(id: string): void {
    this.taskService.delete(id).subscribe(
      (task) => {
        this.notificationService.notificateSuccess(
          'Atividade removida com sucesso'
        );
        this.router.navigate(['/tasks']);
      },
      (e) =>
        this.notificationService.notificateFailure('Falha ao remover atividade')
    );
  }

  canChangeStatus() {
    return (
      this.sharedService.getUserLogged()._id === this.task.creatorUser._id ||
