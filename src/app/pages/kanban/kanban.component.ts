...
export class KanbanComponent {
  ...
  buildKanban(tasks: Task[]) {
    this.tasks = tasks;
    this.createdTasks = tasks.filter(
      (task) => task.status === getStatusFromEnum(Status.CRIADA)
    );
    this.created = this.createdTasks;

    this.todoTasks = tasks.filter(
      (task) => task.status === getStatusFromEnum(Status.EM_ESPERA)
    );
    this.todo = this.todoTasks;

    this.doingTasks = tasks.filter(
      (task) => task.status === getStatusFromEnum(Status.EM_ANDAMENTO)
    );
    this.doing = this.doingTasks;

    this.doneTasks = tasks.filter(
      (task) => task.status === getStatusFromEnum(Status.CONCLUIDA)
    );
  ...
  drop(event: CdkDragDrop<string[]>) {
    ...
    const diferences = this.checkForDiferences(
      oldCreated,
      oldTodo,
      oldDoing,
      oldDone
    ...
    if (task !== undefined) {
      ...
      this.taskService
        .updateTaskStatus(task._id, taskStatus, true)
        .subscribe((c) => {});
    }
  }
  ...
  checkForDiferences(
    ...
    let missingCreated = oldCreated.filter(
      (item) => this.created.indexOf(item) < 0
    );
    let missingCreatedReverse = this.created.filter(
      (item) => oldCreated.indexOf(item) < 0
    );
    let diferenceCreated = [...missingCreated, ...missingCreatedReverse];

    let missingTodo = oldTodos.filter((item) => this.todo.indexOf(item) < 0);
    let missingTodoReverse = this.todo.filter(
      (item) => oldTodos.indexOf(item) < 0
    );
    let diferenceTodo = [...missingTodo, ...missingTodoReverse];

    let missingDoing = oldDoings.filter((item) => this.doing.indexOf(item) < 0);
    let missingDoingReverse = this.doing.filter(
      (item) => oldDoings.indexOf(item) < 0
    );
    let diferenceDoing = [...missingDoing, ...missingDoingReverse];

    let missingDone = oldDones.filter((item) => this.done.indexOf(item) < 0);
    let missingDoneReverse = this.done.filter(
      (item) => oldDones.indexOf(item) < 0
    );
